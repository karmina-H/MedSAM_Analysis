...
def infer_3d(predictor, img_npz_file, gts_file, propagate, model_cfg, pred_save_dir):
    ...
    print(f"image_3D: {img_3D.shape}") # ->image_3D: (49, 512, 512)
    # 픽셀크기 256보다크면 정규화
    ...
    print(f"z_range: {z_range.shape}") # ->z_range: (3,)
    ...
    print(f"z_indices:{z_indices}") # -> z_indices:[20 42]
    print(f"slice_idx:{slice_idx}") # -> slice_idx:31

    #여기서 Total segmentator를 이용해서 전체 슬라이스를 한번 분할해주는 거로
    if not propagate:
        # predicting only middle slice
        # 전체가 아니라 슬라이스 하나에서만 segmentation하는것.
        img = img_3D[slice_idx] / 255. # -> 0~1로 값 정규화( ai쓸때 안정적으로 동작하게하려고)
        img = img.astype(np.float32)
        predictor.set_image(img) # 예측기에 img세팅
        # 예측기로 mask생성.
        ...
        if len(masks.shape) == 3: # single bounding box
            masks = [masks]
        # 경계박스 여러개 가지고 생성했을때, 마스크가지고 라벨 부여
        for idx, mask in enumerate(masks, start=1):
            segs_3D[slice_idx, (mask[0] > 0.0)] = idx
        # 결과 마스크를.npz 즉 넘파이배열의 압축파일형태로 저장해줌
